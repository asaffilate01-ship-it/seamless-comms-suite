...
Version 0.3 also adds an **AI & connectors** specialist hub. A transaction or discovery agent can inspect a deterministic planning preview and propose follow-up tasks, while a finance agent uses the existing financial report. Models do not bypass plan versioning, evidence gaps or independent acceptance. See `AI-LAYER-SETUP.md` for provider setup, routes, scopes and limits.
